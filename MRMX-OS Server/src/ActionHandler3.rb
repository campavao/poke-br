...
class ActionHandler
	...
	#----------------------------------------------------------------------
	# Executes requesting a battle with a player.
	#  username - username of the other player
	# Returns: Action result of this action.
	#----------------------------------------------------------------------
	def prepare_battle_request(username)
		@args = {'PLAYER' => username}
		check = RMXOS.server.sql.query("SELECT COUNT(*) AS count FROM users WHERE username = '#{RMXOS.sql_string(username)}'")
		hash = check.fetch_hash
		# player doesn't exist
		...
		@client.send('TBS', 0, username)
		# notify other player to begin the trade, other player is the host
		@client.sender.send_to_id(client.player.user_id, RMXOS.make_message('TBS', 1, @client.player.username))
		# Below is old request logic
		# @@mutex.synchronize {
		# 	# check already existing actions first
		# 	# actions = @pending.find_all {|action| action.type == Action::TYPE_BATTLE_REQUEST && action.sender_id == client.player.user_id}
		# 	# if actions.size > 0 # there is already a buddy request from this player
		# 	# 	actions.each {|action| self._execute_yes(action.action_id)}
		# 	# 	self.clear_data
		# 	# 	return RMXOS::Result::IGNORE
		# 	# end
		# 	# prepare request
		# 	sender_messages = Action::MessagePack.new(RMXOS::Data::BattleRequested_PLAYER,
		# 		'', RMXOS::Data::BattleNoRequest_PLAYER, @args)
		# 	receiver_messages = Action::MessagePack.new(RMXOS::Data::BattleRequest_PLAYER,
		# 		'', RMXOS::Data::BattleNoRequest, {'PLAYER' => @client.player.username})
		# 	self._create_interaction(Action::TYPE_BATTLE_REQUEST, sender_messages, [client], @client.player.username, receiver_messages)
		# }
		return RMXOS::Result::SUCCESS
	end
	#----------------------------------------------------------------------
	...
	#----------------------------------------------------------------------
	# Executes a battle request to start the battle.
	#  action - the action object for this action
	#----------------------------------------------------------------------
	def execute_battle_request(action)
		other_user_name = action.data
		# cancel other trade requests
		actions = @sent.find_all {|action| action.type == Action::TYPE_BATTLE_REQUEST}
		actions.each {|action| self._cancel_sent_request(action.action_id)}
		# notify this player to begin the trade
		@client.send('TBS', 0, other_user_name)
		# notify other player to begin the trade, other player is the host
		@client.sender.send_to_id(action.sender_id, RMXOS.make_message('TBS', 1, @client.player.username))
		return RMXOS::Result::SUCCESS
	end
