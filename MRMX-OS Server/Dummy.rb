...
module RMXOS
  ...
  module Data
    ...
    RegisterUserPass      = 'Enter a username.'
    Repeat                = 'Repeat:'
...
module RMXOS
  ...
  class Network
    ...
    def update_map_players
      ...
      @map_players.each_value {|player|
      #Kernel.pbNoticeOtherPlayer(player) 
...
class Scene_UserPass < Scene_Network
  ...
  #----------------------------------------------------------------------------
  # Checks if user is facing another player.
  #----------------------------------------------------------------------------
  ...
  def save_user_pass
    ...
    if RMXOS::Options::REMEMBER_LOGIN
      # create data stream
      stream = "#{@username_window.text}\t#{@username_window.text}"
      # zip stream
...
class Scene_Login < Scene_UserPass
  ...
  def create_windows
    ...
    begin
      ...
      if lines.size == 2
        ...
        @username_window.text, @username_window.text = lines
  ...
  def submit_to_server
    # try to login
    $network.try_login(@username_window.text, @username_window.text)
...
class Scene_Register < Scene_UserPass
  ...
  def submit_to_server
    ...
    #   @help_window.set_text(RMXOS::Data::PassNotRepeated)
      ...
      # try to register
      $network.try_register(@username_window.text, @username_window.text)
      # show waiting message
